...
| 3 | Domain invariants + concurrency tests | ✅ Done — landed on `playback-hardening-phase-3-invariants` 2026-06-12 |
| 4 | Server-side position interpolation + heartbeat sanity bound | ✅ Done — landed on `playback-hardening-phase-4-position` 2026-06-12 |
| 5 | Domain events + outbox-style broadcasts | ⊘ Superseded by the actor refactor (2026-06-12) — see note |
| 6 | Client-side command idempotency | ✅ Done — pre-implemented (adapted) in the actor refactor; see note |
...
- **Phase 5 — superseded by the actor refactor (no code; decided 2026-06-12).** The phase targeted a world where `PlaybackSessionHub` manually broadcast after each message handler, so a forgotten broadcast would leave clients stale. That world is gone: **all broadcasting is centralized in `PlaybackUserActor`** (`BroadcastSessionAsync` for material `state`, `BroadcastTickAsync` for heartbeats, `BroadcastDevicesAsync` for the device list), and **every session mutation must enqueue through the actor** (single-writer), so the actor always broadcasts after a material change. A future REST endpoint or job that mutates the session must go through the actor too — it gets the broadcast for free. The phase's "stop relying on every mutation site to remember to broadcast" goal is therefore already met. The doc's specific mechanism — *the store dispatches domain events to Infrastructure broadcast handlers* — also **cannot** be built as written: broadcasting needs `PlaybackConnectionStore`, which lives in `ColdHarbour.Api`, and `Infrastructure` must not reference `Api`. A domain-events seam (events on the aggregate, drained by the actor) was considered but skipped: it would fight the `tick`/`state` split and add surface to the broadcast core for near-zero behavioural gain. If a multi-instance api ever needs cross-process broadcasts, revisit this as a persistent outbox (see the post-migration bridge), not an in-process MediatR dispatcher.
- **Phase 6 — done, pre-implemented (adapted).** Client-side command idempotency is live from the actor refactor. The frontend `send()` attaches a `commandId` (UUID, with the non-secure-context fallback). The actor keeps a bounded per-user LRU of seen command ids (`_seenCommandIds`/`_seenCommandIdOrder`, **cap 256** — the doc said 128) and, on a duplicate, **emits a `noop` `command-ack`** to the originating socket and skips the handler (the doc proposed a full session re-broadcast for convergence; the `noop` ack is the lighter, already-shipped equivalent — the sender learns its retry was absorbed). There is **no separate `CommandIdempotencyStore` Application service** — the dedup lives inline in the actor, the single command chokepoint. Covered by `PlaybackUserActorTests.CommandId_Duplicate_Produces_Noop_Ack_Not_Applied` + `CommandId_Applied_Ack_Is_Unicast_To_Source_Socket`, and the frontend attaches `commandId` on every send. The LRU is in-memory only (lost on restart), as the doc specifies.
- **Phase 4 — done (adapted).** `PlaybackSession.CurrentPositionMs(now)` (interpolates `PositionMs` forward by wall-clock while playing, frozen while paused, clamped against clock-skew rewind) added and surfaced on `PlaybackSessionDto` as `currentPositionMs` (computed in both the actor's `ToDto` broadcast and the REST query). **REST endpoint already existed** as `GET /api/session` (`SessionController` → `GetActiveSessionQuery`); the doc's `/api/playback/session` reconciles to it — just enriched the DTO (no new route, posture unchanged). Heartbeat sanity bound: `PlaybackSession.RecordHeartbeat(positionMs, maxForwardDriftMs)` accepts only `[PositionMs − 250, PositionMs + maxDrift]`, else drops (no `UpdatedAt` bump, returns `false`); the heartbeat path now routes through it via `UpdatePlaybackPositionCommand` (gained a `MaxForwardDriftMs` param, default 5000). The **actor supplies the configured drift** (`COLDHARBOUR_HEARTBEAT_MAX_DRIFT_MS`, default 5000) from `PlaybackLimits` — keeping the handler DI-free so the existing actor test SPs need no new registration. `trackEnded` stays exempt (it sets `PositionMs = 0` directly, never through the heartbeat path). **Frontend:** unchanged — the DTO field is additive (ignored by the WS client, which already interpolates from `positionMs + updatedAt`).
- **Phase 3 — done (adapted).** `ClaimActiveIfNone` lived in the **10 command handlers** (not the hub — the doc predates the actor refactor); centralized into `PlaybackSession.ApplyTransport(senderDeviceId, mutate)` so `git grep ClaimActiveIfNone ColdHarbourBackend/src` now hits **only** `PlaybackSession.cs`. Domain hard cap `PlaybackSession.MaxQueueSize = 1000` + `QueueTooLargeException`, thrown by `SetQueue` and `AddToQueue`. **Reconciliation with the WS-hardening validator:** the configurable `COLDHARBOUR_WS_MAX_QUEUE_SIZE` edge cap (validator) already rejects oversized `setQueue` before the handler; the domain cap is the absolute backstop and specifically closes the **`addToQueue` incremental-growth hole** the validator does not bound. The actor catches `QueueTooLargeException` → logs + drops the frame (consistent with the `ValidationException` path); the doc's client `{ type: "error", code: "queue_too_large" }` frame + frontend toast were **deferred** (not in the Phase-3 DoD, and the server is already protected). New `ConcurrentCommandPropertyTests` runs 50 deterministic seeds × 60 random commands from 3 devices through the real handlers, asserting every invariant (empty-queue ⇒ no track, `0 ≤ QueueIndex < Count`, `TrackId == Queue[QueueIndex]`, `Count ≤ MaxQueueSize`, active ∈ known devices) at each step.
