...
describe('QueuePanelComponent', () => {
  ...
  describe('empty state', () => {
    it('shows the empty manifest section when no items', () => {
      setUp([]);
      expect(fixture.debugElement.query(By.css('.stage__queue-empty'))).toBeTruthy();
    });

    it('hides the list when no items', () => {
      setUp([]);
      expect(fixture.debugElement.query(By.css('.stage__queue-list'))).toBeNull();
    });
  ...
  describe('with items', () => {
    const THREE_ITEMS: QueueItem[] = [
      makeItem({ index: 0, name: 'Alpha', author: 'Artist A', isCurrent: true }),
      makeItem({ index: 1, trackId: 'track-2', name: 'Beta', author: 'Artist B' }),
      makeItem({ index: 2, trackId: 'track-3', name: 'Gamma', author: 'Artist C' }),
    ];

    it('renders one row per item', () => {
      setUp(THREE_ITEMS);
      expect(fixture.debugElement.queryAll(By.css('.stage__queue-row')).length).toBe(3);
    });

    it('shows the track count badge', () => {
      setUp(THREE_ITEMS);
      const badge = fixture.debugElement.query(By.css('.stage__queue-track-count'));
      expect(badge).toBeTruthy();
      expect((badge.nativeElement as HTMLElement).textContent).toContain('3 TRACKS');
    });

    it('hides the empty state when items are present', () => {
      setUp(THREE_ITEMS);
      expect(fixture.debugElement.query(By.css('.stage__queue-empty'))).toBeNull();
    });
    ...
    it('applies --current modifier class only to the current item', () => {
      ...
      expect(rows[0].nativeElement.classList).toContain('stage__queue-row--current');
      expect(rows[1].nativeElement.classList).not.toContain('stage__queue-row--current');
      expect(rows[2].nativeElement.classList).not.toContain('stage__queue-row--current');
    });

    it('disables move-up on the first item', () => {
      setUp(THREE_ITEMS);
      const btns = fixture.debugElement.queryAll(By.css('.stage__queue-row'))[0]
    ...
    it('disables move-down on the last item', () => {
      setUp(THREE_ITEMS);
      const btns = fixture.debugElement.queryAll(By.css('.stage__queue-row'))[2]
        .queryAll(By.css('.stage__queue-btn'));
    ...
    it('does not disable move-up for a middle item', () => {
      setUp(THREE_ITEMS);
      const btns = fixture.debugElement.queryAll(By.css('.stage__queue-row'))[1]
        .queryAll(By.css('.stage__queue-btn'));
  ...
  describe('outputs', () => {
    ...
    it('emits close when the close button is clicked', () => {
      ...
      fixture.debugElement.query(By.css('.stage__queue-close')).nativeElement.click();
    ...
    it('emits clear when the clear button is clicked', () => {
      ...
      fixture.debugElement.query(By.css('.stage__queue-clear')).nativeElement.click();
  ...
  describe('mobilePage input', () => {
    it('host does not have stage__queue--mobile-page class by default', () => {
      ...
      ).not.toContain('stage__queue--mobile-page');
    });
    ...
    it('host gains stage__queue--mobile-page class when mobilePage is true', () => {
      ...
      expect(
        (fixture.nativeElement as HTMLElement).classList,
      ).toContain('stage__queue--mobile-page');
    });
  });
});
