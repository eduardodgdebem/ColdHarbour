...
| `COLDHARBOUR_ACTIVE_DEVICE_TTL_SECONDS`             | compose           | Seconds the active device may be gone (no live socket) before the session releases ownership. Default `30` |
| `COLDHARBOUR_HEARTBEAT_MAX_DRIFT_MS`                | compose           | Max forward jump (ms) a single heartbeat may report past the last position before it is rejected. Default `5000` |
...
**Active-device guard (phase 2 narrowing):** the guard now applies **only** to `heartbeat` and `stop` — these carry position truth / destructive intent and must come from the device actually playing audio. Transport commands (`pause`, `resume`, `next`, `previous`, `seek`, `setQueue`) explicitly accept any device's deviceId so any client can drive playback on the active device (Spotify-Connect semantics). The guard (`PlaybackUserActor.IsActiveDevice`) **fails closed** (WS hardening phase 3): an empty deviceId or a session with no active device yet both return `false`, so heartbeat/stop can't feed or clear an ownerless session.

**Sender-device authentication (WS hardening phase 3):** before dispatch, the actor rejects any command whose claiming/transport deviceId is `Guid.Empty` or is not a registered device for the user (`IDeviceRepository.ExistsForUserAsync`). This closes claim-active spoofing — a client can no longer pump an arbitrary UUID and become active. Rejected commands are logged at `Warning` ("unknown sender device") and dropped; the socket is **not** closed. Validation lives at the actor chokepoint (`PlaybackUserActor.SenderDeviceToValidate`), the single client entry for transport commands. `heartbeat`/`stop` rely on the active-device guard instead; `resync` (read-only) and `setRepeatMode`/`setShuffle` (no device) are exempt; `pause`/`resume` are validated only when they carry a deviceId.

**Sender-claim-active rule:** when a transport command arrives from a validated device and `session.ActiveDeviceId` is null, the sender's deviceId is assigned as active. Existing active devices are not displaced — only `transfer` moves activity.
...
- `PlaybackSession { UserId, ActiveDeviceId, TrackId, PositionMs, IsPlaying, Queue, QueueIndex, RepeatMode, Shuffle, UpdatedAt }` — in-memory, one per user. `Queue` is an ordered list of `Guid` track IDs; `QueueIndex` points at the current item. Invariant: when `Queue` is non-empty, `0 ≤ QueueIndex < Queue.Count`. `SetQueue` also primes `TrackId = Queue[startIndex]`, `IsPlaying = true`, `PositionMs = 0` (subsumes the retired `Start`). `AdvanceNext` / `AdvancePrevious` wrap. `AdvanceAfterEnd` honors `RepeatMode` (`Off` stops at end of cycle, `All` wraps, `One` restarts the same track) and `Shuffle` (stable Fisher-Yates order, re-seeded on `SetQueue` / `SetShuffle(true)`). `ClaimActiveIfNone(deviceId)` installs the sender as active when no device owns playback; transport command handlers go through `ApplyTransport(senderDeviceId, mutate)`, the single place that does claim-if-none + the mutation (so `ClaimActiveIfNone` is never called outside `PlaybackSession`). `DemoteActiveDevice()` releases ownership when the active device has gone stale (Playback-hardening Phase 1). A hard `MaxQueueSize = 1000` cap is enforced by the aggregate — `SetQueue`/`AddToQueue` throw `QueueTooLargeException` past it (the configurable `COLDHARBOUR_WS_MAX_QUEUE_SIZE` edge cap should stay ≤ this hard cap); the actor drops the offending frame and logs.
