...
- **Phase 5 — done.** `ColdHarbour.Application/Playback/Validators/PlaybackCommandValidators.cs` holds one `AbstractValidator` per hub-dispatched command (SetQueue, Seek, Next/Previous, Pause/Resume, Stop, SetRepeatMode, TrackEnded, Add/Remove/Reorder/ClearQueue). They are auto-discovered by the existing `AddValidatorsFromAssembly` + `ValidationBehavior` pipeline — no plumbing needed. `SetQueueCommandValidator` reads the new `PlaybackLimits.MaxQueueSize` (bound from `COLDHARBOUR_WS_MAX_QUEUE_SIZE`, default 1000, in `Program.cs`; a default `PlaybackLimits` is also registered in `AddApplication` so the Application DI is self-sufficient). The actor's pump gained a dedicated `catch (ValidationException)` that logs a Warning with the message type + errors and drops the frame — the socket is never closed. **Frontend:** left unchanged — the doc marks the client-side truncation as defensive/optional, and the server is now the authoritative enforcement point.
  - **Side note for the pre-existing red:** the 7 failing `PlaybackUserActorTests` fail because their local `BuildServices` registers MediatR without `IPlaySessionTimeline`/`ITrackRepository`, so the handlers can't be constructed and `changed`/`Revision` stay 0. The new `PlaybackHubValidationTests` build their provider via the real `AddApplication()` and pass; fixing the older harness is still deferred to a separate change per the note below.
