...
| 5 | FluentValidation on hub-dispatched commands | ✅ Done — landed on `ws-hardening-phase-5-hub-validation` 2026-06-12 |
...
- **Phase 5 — done.** `ColdHarbour.Application/Playback/Validators/PlaybackCommandValidators.cs` holds one `AbstractValidator` per hub-dispatched command (SetQueue, Seek, Next/Previous, Pause/Resume, Stop, SetRepeatMode, TrackEnded, Add/Remove/Reorder/ClearQueue). They are auto-discovered by the existing `AddValidatorsFromAssembly` + `ValidationBehavior` pipeline — no plumbing needed. `SetQueueCommandValidator` reads the new `PlaybackLimits.MaxQueueSize` (bound from `COLDHARBOUR_WS_MAX_QUEUE_SIZE`, default 1000, in `Program.cs`; a default `PlaybackLimits` is also registered in `AddApplication` so the Application DI is self-sufficient). The actor's pump gained a dedicated `catch (ValidationException)` that logs a Warning with the message type + errors and drops the frame — the socket is never closed. **Frontend:** left unchanged — the doc marks the client-side truncation as defensive/optional, and the server is now the authoritative enforcement point.
  - **Side note for the pre-existing red:** the 7 failing `PlaybackUserActorTests` failed because their local `BuildServices` registered MediatR without `IPlaySessionTimeline`/`ITrackRepository`, so the handlers couldn't be constructed and `changed`/`Revision` stayed 0. **Now fixed** (see below).

**Pre-existing red — resolved (2026-06-12).** `ColdHarbour.Api.IntegrationTests/Playback/PlaybackUserActorTests` previously had 7 failing tests (revision/ack/tick assertions found `Revision == 0`) because the test's `BuildServices` registered MediatR but not `IPlaySessionTimeline` (and its `ITrackRepository` dependency), so command handlers threw on construction, the pump swallowed the error, and no material command incremented `Revision`. Fixed by registering `ITrackRepository` (a no-op double) + `IPlaySessionTimeline → PlaySessionTimeline` in that harness. All 14 `PlaybackUserActorTests` now pass; the whole non-container Playback set (actor + guard + validation + shutdown) is green at 26/26.
